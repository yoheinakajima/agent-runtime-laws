...
until its footprint is resolved.  This property deliberately does not assert that a
retrospectively discarded source suffix left the original world unchanged.
\end{definition}

\paragraph{Remark (assessor scope).}
The v0 assessor takes the target-environment premise as an input assumption; it
does not verify a snapshot or fresh-environment attestation.  Its verdict is
relative to the immutable log snapshot supplied at assessment time.  A
production caller must bind the assessment to a source high-water mark and
establish log closure, quiescence, or another explicit freshness policy before
acting on it.  The companion bridge in Section~\ref{sec:validation}
authenticates and fork-binds a caller-issued environment assertion, but does not
independently validate the assertion's contents or rule out a later-arriving
hazard.  The language-neutral assessor remains verifier-agnostic.
...
Closed assessor return values are typeset as \code{Sound}, \code{Conditional},
and \code{Unsound}; lowercase words retain their ordinary predicate meaning.
...
all $|L|+1$ cuts.  Table~\ref{tab:cut-through-request} separates the three
boundary regions created by one request/outcome pair.
