...
The transition and settlement boundary consists of 594 source lines in
\code{Domain.fs} and \code{Kernel.fs}.  Fork and grade analyzers add 799 lines,
for 1,393 core-law lines.  Validation and conformance adapters are separate.
This exceeds the initial 1,000-line whole-kernel target.  We report the miss
explicitly rather than compressing away lifecycle integrity, property-relative
findings, or lineage-bearing branches.
...
\begin{itemize}[leftmargin=*]
  ...
  \item 44 passing tests, including FsCheck properties and named fixtures;
  ...
  \item a findings log that records failed laws as they are discovered.
\end{itemize}
...
\subsection{Property-based laws}
...
\begin{itemize}[leftmargin=*]
  ...
  \item identity, nested-collapse, projection, and suffix laws for forks;
  \item verdict matrices across generated footprint and replay-source values;
  \item grade-chain lattice laws and prerequisite enforcement.
\end{itemize}
...
facts.  Duplicate or non-contiguous normalized identity remains a blocker.
...
for suffixes are accumulated in reverse.  The validator reports Sound,
Conditional, and Unsound totals for ProjectionReplay, ExternalContinuation,
and CounterfactualWorld, plus the number of runs containing an unsound
counterfactual cut.
...
timestamp.  Child run ID and the database's global autoincrement sequence are
excluded by definition.  The audit separately counts retained external
requests and unresolved boundaries.
