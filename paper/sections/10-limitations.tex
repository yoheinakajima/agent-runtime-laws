...
The propositions are encoded as executable laws and checked over finite
generators and artifacts.  A passing FsCheck property is not a proof over all
behaviors, schedulers, or logs.  This is a conscious genre choice, but it limits
the strength of words such as ``law.''  Future work may mechanize selected
definitions after the operational contracts stabilize.
...
not modeled as a separate production activation dimension.
...
independently authenticated well-formed prefix.  The projection suffix law is
...
not by an actual public fork with a no-reexecution receipt.

\paragraph{Bridge provenance.}
The bridge artifact is locally content-addressed but lives in an unborn Git
repository with no remote.  A fresh external reviewer cannot reconstruct the
source revision.  The result should remain a draft finding until the artifact
and generator are committed or published.  This limitation is particularly
important because provenance is part of the paper's thesis.
...
The core-law implementation is 1,393 lines rather than the desired roughly
1,000.  The transition/settlement boundary remains 594 lines.  The additional
fork and grade diagnostics improved fail-closed behavior, but the overage may
still reduce readability.  Refactoring should preserve named findings and law
coverage rather than optimizing only for a line count.
