...
$\Omega_{profile}$.  The public bridge v0.2 receipt records observable tags,
but we do not retroactively impute them to older corpus events.  Narrower corpus
claims require adapter-supplied tags and new conformance vectors.
...
one actual bridge fork after a committed recorded oracle effect.  Its receipt
verifies zero inherited external calls and an HMAC-authenticated fixture
environment under a configured trust root.  This closes the prior absence of a
public no-reexecution mechanism test, but only for one deterministic offline
oracle, one child, and a deliberately published fixture key.  It does not
establish real-provider behavior, production attestor identity, environmental
fidelity, or prevalence across production forks.
