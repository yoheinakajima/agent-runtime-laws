...
\paragraph{Kernel/production read timing.}
The executable kernel snapshots state for all activations enabled by one
trigger.  The pinned ActiveGraph source computes the match set once but rebuilds
each handler view after prior handlers may have emitted and synchronously
projected events.  The source audit identifies this additional ordering seam;
the checked kernel properties do not model it, and no production behavior
inventory establishes whether it changes an observed run.
...
The generic F\# effect descriptor and legacy corpus adapters have no per-effect
observable-set label.  Their CounterfactualWorld results therefore use the
selected profile's entire recognized external surface as
$\Omega_{profile}$.  The public bridge v0.2 receipt records
observable tags, but we do not retroactively impute them to older corpus events.
Narrower corpus claims require adapter-supplied tags and new conformance
vectors.
...
\paragraph{Assessment snapshot and delayed hazards.}
Every verdict is relative to the finite log snapshot presented to the assessor.
An asynchronous failure, compensation result, webhook timeout, or other hazard
may be appended after that assessment and downgrade a previously clean prefix.
The artifact does not model an observation-delay distribution or prove that a
source log is closed.  Production admission therefore requires a receipt bound
to a source high-water mark plus an explicit closure, quiescence, or freshness
policy and a recheck immediately before continuation.  A time window alone is
an operational assumption, not proof that no later hazard exists.
...
public bridge fixture uses a deterministic offline fixture and explicitly makes
...
boundary, and all 160,076 Sound corpus ExternalContinuation cuts precede the
first classified effect.  The companion public conformance fixture now covers
one actual bridge fork after a committed recorded oracle effect.  Its generator
observes no second fixture-oracle call, while its verifier checks receipt/log
consistency and a signed, fork-bound caller assertion under a configured trust
root.  This exercises the previously absent public no-reexecution mechanism,
but only for one deterministic offline oracle, one child, a process-produced
counter, and a deliberately published fixture key.  The verifier does not
inspect the asserted environment.  The result does not establish real-provider
behavior, production attestor identity, environmental fidelity, or prevalence
across production forks.
...
The public corpus, observed forks, and bridge implementation all descend from
ActiveGraph.  The language-neutral vectors support other implementations, but
no LangGraph, Temporal, or other independent runtime trace has yet been
normalized.  Generalization beyond the source family is therefore conceptual
rather than empirically demonstrated.

\paragraph{Baseline conformance-vector coverage.}
The seven language-neutral vectors cover the boundary cases listed in
Table~\ref{tab:vector-coverage}; they do not enumerate the full verdict matrix.
Only the unknown-inherited-footprint obligation has a dedicated Conditional
vector.  Dedicated portable vectors for no-reexecution, discarded-effect
reconciliation, compensation, and failed-effect reconciliation remain future
work, although the F\# suite exercises those obligations directly.
