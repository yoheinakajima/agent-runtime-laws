...
Set inclusion supplies meet and join, so the five grades form a finite chain and
therefore a lattice.  The implementation checks idempotence, commutativity,
absorption, and lower/upper-bound laws over every pair of grades.  This result
is definitional rather than a discovery about all possible taxonomies.  A
future capability taxonomy could contain incomparable evidence sets; the code's
comparison function refuses incomparable grades rather than inventing an
order.
...
divergence.  These requirements prevent an event-name coincidence from raising
the grade.
...
hazard event adds a blocker, so the extended log grades Envelope.
\end{counterexample}
