...
post-oracle conformance fork.  The sealed Synthetic Players capsule contains an
ActiveGraph SQLite store.  The activegraph-bridge fixture executes an actual
bridge fork after a committed recorded fixture-oracle effect.  Both contain
synthetic experimental content; ``real'' here means actual runtime artifacts
rather than invented shape fixtures.  Neither is independent of the ActiveGraph
family, and we do not claim human behavioral validity or real-model performance.
...
observation.  The 36,251 ExternalContinuation \code{Unsound} cuts equal the number of
...
The 120,969 \code{Conditional} continuation cuts also have a direct per-run
identity.  For a request-bearing run with $n_r$ events, first-request ordinal
$q_r$, and $m_r$ requests, the $q_r$ earlier cuts are \code{Sound}, each request
contributes one open-boundary \code{Unsound} cut, and the remaining
$n_r+1-q_r-m_r$ closed post-response cuts are \code{Conditional}.  Summing that
last term yields 120,969; these cuts retain at least one committed OneShot oracle
interaction and require serving recorded results without another call.

CounterfactualWorld is stricter and has a separate structural identity.  In
runs whose requests are all closed, every cut before the final response has a
committed oracle interaction in the discarded suffix; the sum of those final
response ordinals is 141,545.  In the 24 runs ending with an unmatched request,
all $n_r+1$ cuts are \code{Unsound}, adding 1,349.  Thus
$141{,}545+1{,}349=142{,}894$.  The union contains 4,923 affected runs: 4,919
have at least one response, four open runs have none, and the other 20 open runs
are already in the first set.
...
The companion activegraph-bridge artifact is public in the v0.2.0 release and
on the default branch at revision \code{843824a44d48}.  The complete
\href{https://github.com/yoheinakajima/activegraph-bridge/tree/843824a44d48d816779fc0c08580ae06108fe7b6/evidence/post-oracle-fork-v1}{public post-oracle fixture and verifier}
are revision-pinned.
...
runtime fingerprints, and target-environment claims.  The fixture generator
directly observes the oracle-call counter before and after the child operation.
The verifier checks that process-produced counter, the receipt/log hashes, and
an HMAC-SHA256 signature under a configured trust root whose caller-issued
claims include the child, cut, prefix, and target fingerprint.
...
This exercises the previously missing implementation path for a public
conformance case.  In this deterministic offline fixture, an inherited recorded
oracle outcome crosses a fork boundary with no second observed fixture-oracle
call.  A caller-issued target-environment assertion is signature-checked and
bound to that fork under a configured trust root; the verifier does not inspect
or independently establish the asserted environment contents.  The HMAC key is
deliberately published as a fixture key.  The result does not attest a real
...
Completed for the public corpus.  The capsule yields Observed for 5,540 logs
because verification is external to each run log.  The result supports
log-derived grading rather than profile-derived labels and separates a sealed
artifact's verification from evidence carried by an individual run.
...
fork after a committed recorded oracle call.  Its generator observes no second
fixture-oracle call; its verifier checks receipt/log consistency and a
fork-bound caller assertion under a public conformance key.  It is conformance
evidence, not a real-provider or production-attestor result.
...
prefix copying, and one public offline post-oracle fixture in which no second
fixture-oracle call was observed and a caller environment assertion was
fork-bound.  It does not support an unrestricted claim that any historical cut
is safely executable or environmentally counterfactual.  Those stronger claims
require effect-complete evidence and a property-specific precondition.
