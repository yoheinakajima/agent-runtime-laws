...
We evaluate the contracts against one public trace corpus and retain one local
instrumented study as illustrative evidence.  The public, sealed Synthetic
Players capsule contains an ActiveGraph SQLite store.  The local
activegraph-bridge executive study records explicit boundary verification but
has no public source remote.  Both contain synthetic experimental content;
``real'' here means actual runtime artifacts rather than invented shape
fixtures.  Neither corpus is independent of the ActiveGraph family, and we do
not claim human behavioral validity or real-model performance.
...
the 617 runs with no classified request.  They do not empirically validate a
post-oracle continuation, target-environment attestation, or zero-reexecution
receipt.  Generated properties and named fixtures exercise post-oracle verdicts
and typed no-reexecution obligations.  The target-environment premise itself is
not discharged by an attestation mechanism in v0 and remains unvalidated.
...
\subsection{Illustrative local activegraph-bridge executive study}
...
\code{submit\_recommendation} is in scope.  A narrower observable set could
change the world verdicts, but v0 has no per-effect observable tags and does not
report such a result.
