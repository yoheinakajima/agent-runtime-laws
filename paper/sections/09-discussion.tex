...
others.  The Synthetic Players capsule passes an external byte-exact verifier,
but its individual logs contain no verification attestation, so they remain
Observed under a log-alone grade.  Every prefix projects, but cuts through
requests fail execution continuation.  The public bridge fixture then shows a
different predicate again: one committed recorded oracle result is inherited
across a fork with zero new calls under a fork-bound caller assertion.  These
are not contradictions; they are different predicates.
...
public activegraph-bridge v0.2 implementation emits a hash-bound
...
graph may be derived; ordering and effect contracts remain non-eliminable
obligations of the operational semantics.
