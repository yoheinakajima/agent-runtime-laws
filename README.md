...
- Pairwise-disjoint declared writes restore confluence for the deliberately
  restricted single-trigger writer family currently generated by the suite.
  This is not yet claimed as a minimal condition for arbitrary behaviors.
...
The checked-in evidence fixtures are synthetic, sanitized, and
shape-conformant. They verify the normalization and hash-binding machinery;
they are not represented as production-log validation.

Real ActiveGraph, synthetic-players, and activegraph-bridge traces are loaded
read-only by path and are not copied into this repository unless their
publication and sanitization status is independently established. See
EVIDENCE.md.
...
Research v0. The public claims remain provisional until the real-log validation
table in EVIDENCE.md is populated and reviewed.
